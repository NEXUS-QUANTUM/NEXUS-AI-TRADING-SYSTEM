...
check_prerequisites() {
    ...
    if [ ! -f "$ENV_FILE" ]; then
        ...
        log_info "Veuillez exécuter ./setup.sh ou copier .env.example vers .env"
        exit 1
    fi
    ...
    if ! command -v node &> /dev/null; then
        log_warning "Node n'est pas installé (le frontend ne pourra pas fonctionner)"
    else
...
main() {
    ...
    log_info "Démarrage du projet à $(date)"
    log_info "Log file: $LOG_FILE"
    
    # Vérifier si déjà en cours d'exécution
    if check_pid; then
        log_warning "Le projet est déjà en cours d'exécution (PID: $(cat $PID_FILE))"
    ...
    echo "4) Démarrer seulement les services Docker"
    echo "5) Démarrer le monitoring"
    ...
    echo -e "\n${YELLOW}Appuyez sur Ctrl+C pour arrêter les services${NC}"
    wait
}
