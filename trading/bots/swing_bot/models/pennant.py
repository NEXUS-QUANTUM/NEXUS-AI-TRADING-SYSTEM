"""
Swing Bot Pennant Model
=========================

This module provides pennant pattern analysis models for the Swing Bot trading system.
"""
...
class PennantPattern:
    """Pennant pattern data structure."""
...
class PennantModel:
    """
    Pennant pattern analysis model.
    
    Identifies and analyzes pennant patterns:
    - Bullish pennants
    - Bearish pennants
    - Flag patterns
    """
    ...
    def analyze(self, df: pd.DataFrame) -> Dict[str, Any]:
        """
        Analyze pennant patterns.
        
        Args:
            df: OHLCV data
            
        Returns:
            Pennant analysis results
        """
    ...
    def _detect_patterns(self, df: pd.DataFrame) -> List[PennantPattern]:
        """
        ...
        Returns:
            List of PennantPattern objects
        """
        patterns = []
        high = df['high'].values
        low = df['low'].values
        ...
        for i in range(len(swing_highs) - 1):
            for j in range(i + 1, len(swing_highs)):
                ...
                patterns.append(PennantPattern(
    ...
    def _calculate_confidence(self, pole_segment: pd.DataFrame, 
        ...
        """
        Calculate pennant pattern confidence.
    ...
    def _calculate_strength(self, pole_segment: pd.DataFrame,
                          flag_segment: pd.DataFrame) -> float:
        """
        Calculate pennant pattern strength.
    ...
    def _generate_signals(self, df: pd.DataFrame,
                         patterns: List[PennantPattern]) -> List[PennantSignal]:
        """
        Generate trading signals from pennant patterns.
        
        Args:
            df: OHLCV data
            patterns: List of pennant patterns
            
        Returns:
            List of PennantSignal objects
        """
        signals = []
        ...
        if latest_pattern.pattern_type == 'bullish':
            if current_price > latest_pattern.breakout_price * (1 + self.breakout_threshold):
                signal_type = 'buy'
                reason = "Bullish pennant breakout detected"
            else:
                return None
        else:
            if current_price < latest_pattern.breakout_price * (1 - self.breakout_threshold):
                signal_type = 'sell'
                reason = "Bearish pennant breakdown detected"
            else:
                return None
        
        signal = PennantSignal(
            symbol=symbol,
    ...
    def _get_market_character(self, df: pd.DataFrame,
                            patterns: List[PennantPattern]) -> str:
        """
        ...
        """
        if not patterns:
            return "No pennant patterns detected"
        
        latest = patterns[-1]
        
        direction_names = {
            'bullish': 'Bullish pennant',
            'bearish': 'Bearish pennant'
        }
...
def create_pennant_model(config: Optional[Dict[str, Any]] = None) -> PennantModel:
    """
    Create a pennant model instance.
    
    Args:
        config: Model configuration
        
    Returns:
        PennantModel instance
    """
    return PennantModel(config)
