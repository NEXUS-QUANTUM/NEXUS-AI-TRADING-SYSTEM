# trading/bots/hedge_bot/hedge_bot_data_notification.py
# Advanced Notification & Alert Management Module for Hedge Bot
# NEXUS AI TRADING SYSTEM - Copyright © 2026 NEXUS QUANTUM LTD

"""
Hedge Bot Data Notification Module - Module avancé de gestion des notifications et alertes
pour le Hedge Bot. Gère les notifications en temps réel, les alertes, les canaux de communication,
les templates de messages et l'escalade des alertes pour l'ensemble du système de hedging.
"""
...
logger = get_logger("hedge_bot_data_notification")
...
class NotificationEngineInterface(ABC):
    """Interface abstraite pour le moteur de notifications."""
...
class NotificationEngine(NotificationEngineInterface):
    """
    Moteur de notifications avancé pour le Hedge Bot.
    Gère les notifications multi-canaux, les templates et les abonnements.
    """
    ...
    async def start(self) -> None:
        """Démarre le moteur de notifications."""
        logger.info("NotificationEngine starting...")
    ...
    async def stop(self) -> None:
        ...
        logger.info("NotificationEngine stopping...")
        self._is_running = False
    ...
    async def create_template(self, template: NotificationTemplate) -> str:
        ...
        if self.data_manager:
            await self.data_manager.store(
                f"notification:template:{template.template_id}",
                template.to_dict(),
                DataType.TEMPLATE
            )
        ...
        while self._is_running:
            ...
            try:
                ...
                with self._notif_lock:
                    ...
                    active_subs = len([s for s in self._subscriptions.values() if s.active])
            ...
            if self.data_manager:
                ...
                if subs_data:
                    for sub_dict in subs_data:
                        sub = self._deserialize_subscription(sub_dict)
                        if sub:
                            with self._sub_lock:
    ...
    def _deserialize_subscription(self, data: Dict) -> Optional[NotificationSubscription]:
        ...
        try:
            return NotificationSubscription(
                ...
                types=[NotificationType(t) for t in data.get("types", [])],
                priorities=[NotificationPriority(p) for p in data.get("priorities", [])],
                tags=data.get("tags", []),
                active=data.get("active", True),
                created_at=datetime.fromisoformat(data.get("created_at", datetime.now(timezone.utc).isoformat())),
                updated_at=datetime.fromisoformat(data.get("updated_at", datetime.now(timezone.utc).isoformat())),
    ...
    async def create_subscription(self, subscription: NotificationSubscription) -> str:
        """Crée un abonnement."""
        with self._sub_lock:
        ...
        if self.data_manager:
            await self.data_manager.store(
                f"notification:subscription:{subscription.subscription_id}",
    ...
    async def get_subscription(self, subscription_id: str) -> Optional[NotificationSubscription]:
...
class NotificationFactory:
    ...
    async def create_engine(
        ...
        """Crée un moteur de notifications."""
