# trading/exchanges/stocks/tradestation.py
# Nexus AI Trading System - TradeStation Stock Trading Module
# Copyright © 2026 NEXUS QUANTUM LTD - All Rights Reserved
# Full version with advanced coding

"""
TradeStation Stock Trading Module

This module provides comprehensive stock trading functionality for the TradeStation
platform via the TradeStation API, including:

- OAuth 2.0 authentication with TradeStation API
...
class TradeStationAccountType(str, Enum):
...
class TradeStationAssetClass(str, Enum):
...
class TradeStationConfig(StockConfig):
    ...
    environment: TradeStationEnvironment = TradeStationEnvironment.SIMULATION
    ...
    def get_base_url(self) -> str:
        ...
        if self.environment == TradeStationEnvironment.SIMULATION:
            return "https://sim-api.tradestation.com/v2"
        else:
            return "https://api.tradestation.com/v2"
    ...
    def get_auth_url(self) -> str:
        ...
        if self.environment == TradeStationEnvironment.SIMULATION:
            return "https://sim-oauth.tradestation.com/oauth"
        else:
            return "https://oauth.tradestation.com/oauth"
    ...
    def get_streamer_url(self) -> str:
        ...
        if self.environment == TradeStationEnvironment.SIMULATION:
            return "wss://sim-streamer.tradestation.com/stream"
        else:
            return "wss://streamer.tradestation.com/stream"
...
class TradeStationAccount(BaseModel):
    ...
    account_id: str
    account_name: str
    account_type: TradeStationAccountType
    account_status: str
    currency: str = "USD"
...
class TradeStationOrder(BaseModel):
    ...
    account_id: str
    symbol: str
    side: TradeStationOrderSide
    order_type: TradeStationOrderType
    status: TradeStationOrderStatus
    time_in_force: TradeStationTimeInForce
    quantity: Decimal
    ...
    def is_open(self) -> bool:
        return self.status in [TradeStationOrderStatus.WORKING, TradeStationOrderStatus.PARTIALLY_FILLED]

    @property
    def is_closed(self) -> bool:
        return self.status in [TradeStationOrderStatus.FILLED, TradeStationOrderStatus.CANCELLED,
                              TradeStationOrderStatus.EXPIRED, TradeStationOrderStatus.REJECTED,
                              TradeStationOrderStatus.STOPPED]


class TradeStationPosition(BaseModel):
    ...
    asset_class: TradeStationAssetClass = TradeStationAssetClass.EQUITY
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
    metadata: Dict[str, Any] = Field(default_factory=dict)
...
class TradeStationBar(BaseModel):
    """TradeStation bar (OHLC)."""
    symbol: str
    open: Decimal
    high: Decimal
    low: Decimal
...
CREATE TABLE IF NOT EXISTS tradestation_orders (
    ...
    INDEX idx_tradestation_orders_symbol (symbol),
    INDEX idx_tradestation_orders_status (status),
    INDEX idx_tradestation_orders_created_at (created_at)
);

-- TradeStation positions
CREATE TABLE IF NOT EXISTS tradestation_positions (
...
class TradeStationOAuth2Client:
    """
    OAuth 2.0 client for TradeStation API authentication.
    """
    ...
    async def initialize(self):
        ...
        logger.info("TradeStationOAuth2Client initialized")
...
# =============================================================================
# MAIN TRADESTATION CLIENT
# =============================================================================

class TradeStationClient(StockExchange):
    """
    Advanced TradeStation stock trading client.
    ...
    def __init__(
        ...
        self.converter.set_broker(StockExchangeType.TRADESTATION)
        
        # OAuth client
        self._oauth = TradeStationOAuth2Client(config)
        
        # Account state
        self._accounts: Dict[str, TradeStationAccount] = {}
        ...
        logger.info("TradeStationClient initialized")
    ...
    async def initialize(self):
        ...
        logger.info("TradeStationClient initialization complete")
    ...
    async def _make_request(
        ...
        """Make an authenticated request to TradeStation."""
        cb_key = "tradestation_order" if "order" in endpoint else "tradestation_market"
            ...
            async with self._session.request(
                ...
                if response.status >= 400:
                    ...
                    if response.status == 403:
                        ...
                        raise StockConnectionError("TradeStation service unavailable")
                    else:
    ...
    async def get_accounts(self, refresh: bool = False) -> Dict[str, TradeStationAccount]:
        """Get all accounts."""
        ...
        try:
            ...
            for item in data.get('accounts', []):
                account = TradeStationAccount(
                    ...
                    account_type=TradeStationAccountType(item.get('accountType', 'Individual')),
    ...
    async def get_default_account(self) -> Optional[TradeStationAccount]:
    ...
    async def place_order(
        ...
        **kwargs
        ...
        try:
            ...
            # Map order type
            ts_type_map = {
                StockOrderType.MARKET: TradeStationOrderType.MARKET,
                StockOrderType.LIMIT: TradeStationOrderType.LIMIT,
                StockOrderType.STOP: TradeStationOrderType.STOP,
                StockOrderType.STOP_LIMIT: TradeStationOrderType.STOP_LIMIT,
                StockOrderType.TRAILING_STOP: TradeStationOrderType.TRAILING_STOP,
            }
            ts_type = ts_type_map.get(order_type, TradeStationOrderType.LIMIT)
            
            # Map side
            ts_side_map = {
                StockOrderSide.BUY: TradeStationOrderSide.BUY,
                StockOrderSide.SELL: TradeStationOrderSide.SELL,
            }
            ts_side = ts_side_map.get(side, TradeStationOrderSide.BUY)
            
            # Build order request
            order_data = {
                "accountId": account.account_id,
                "symbol": symbol,
                "orderType": ts_type.value,
                "orderSide": ts_side.value,
    ...
    async def _connect_streamer(self):
        """Connect to TradeStation streamer."""
        if self._ws_connected:
            return
        
        try:
            token = await self._oauth.get_access_token()
            ws_url = self.config.get_streamer_url()
            ...
            if data.get('status') != 'success':
                raise StockAuthenticationError("Streamer authentication failed")
            
            asyncio.create_task(self._ws_listen_loop())
            
            logger.info("Streamer connected and authenticated")
    ...
    async def _ws_listen_loop(self):
        ...
        while self._ws_connected and not self._shutdown_requested:
            try:
                ...
                logger.warning("Streamer connection closed")
                break
            except Exception as e:
        ...
        if msg_type in self._ws_handlers:
            for handler in self._ws_handlers[msg_type]:
                try:
                    ...
                    logger.error(f"Streamer handler error: {e}")
    
    async def _reconnect_streamer(self):
        """Reconnect streamer."""
    ...
    async def _save_order(self, order: StockOrder, account_id: str):
        ...
        try:
            async with self.pool.acquire() as conn:
                await conn.execute(
                    """
                    INSERT INTO tradestation_orders (
        ...
        try:
            async with self.pool.acquire() as conn:
                for position in self._positions.values():
                    await conn.execute(
                        """
                        INSERT INTO tradestation_positions (
                        ...
                        position.cost_basis,
                        TradeStationAssetClass.EQUITY.value,
        ...
        try:
            async with self.pool.acquire() as conn:
                for account in self._accounts.values():
                    await conn.execute(
                        """
                        INSERT INTO tradestation_accounts (
    ...
    async def shutdown(self):
        """Shutdown the TradeStation client."""
        ...
        logger.info("TradeStationClient shutdown complete")
...
__all__ = [
    ...
    'TradeStationTimeInForce',
    'TradeStationAccountType',
    'TradeStationAssetClass',
    'TradeStationAccount',
    'TradeStationOrder',
    'TradeStationPosition',
    'TradeStationQuote',
    'TradeStationBar',
    'TradeStationOAuth2Client'
]
