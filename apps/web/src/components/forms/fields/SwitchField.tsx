// apps/web/src/components/forms/fields/SwitchField.tsx
'use client';
...
export type SwitchVariant = 'default' | 'success' | 'danger' | 'warning' | 'info' | 'brand' | 'dark' | 'light';
export type SwitchSize = 'xs' | 'sm' | 'md' | 'lg' | 'xl';
export type SwitchLabelPosition = 'left' | 'right' | 'top' | 'bottom' | 'none';
export type SwitchAnimation = 'slide' | 'fade' | 'scale' | 'bounce' | 'none';
...
const SIZE_MAP: Record<SwitchSize, { switch: string; thumb: string; label: string; gap: string }> = {
  xs: {
    switch: 'h-4 w-7',
    thumb: 'h-2.5 w-2.5',
    label: 'text-xs',
...
const VARIANT_MAP: Record<SwitchVariant, { checked: string; unchecked: string; thumb: string }> = {
  default: {
...
const ICON_SIZE_MAP: Record<SwitchSize, string> = {
  xs: 'h-2 w-2',
  sm: 'h-2.5 w-2.5',
  md: 'h-3 w-3',
  lg: 'h-3.5 w-3.5',
...
const ANIMATION_MAP: Record<SwitchAnimation, { duration: number; type: string }> = {
...
export const SwitchField = forwardRef<HTMLDivElement, SwitchFieldProps>(
  (props, ref) => {
    ...
    const validateSwitch = useCallback((val: boolean): { valid: boolean; message: string } => {
      if (customValidate) {
    ...
    const updateChecked = useCallback((val: boolean) => {
      const validation = validateSwitch(val);
      ...
      if (debug) {
        console.log('SwitchField update:', { checked: val, isValid: validation.valid });
      }
    }, [
      validateSwitch,
      isControlled,
    ...
    useEffect(() => {
      if (externalChecked !== undefined && externalChecked !== prevCheckedRef.current) {
        ...
        const validation = validateSwitch(externalChecked);
        setIsValid(validation.valid);
        setValidationMessage(validation.message);
      }
    }, [externalChecked, validateSwitch]);
    ...
    useEffect(() => {
      if (defaultChecked && !isControlled) {
        const validation = validateSwitch(defaultChecked);
        setIsValid(validation.valid);
        setValidationMessage(validation.message);
      }
    }, [defaultChecked, isControlled, validateSwitch]);
    ...
    useImperativeHandle(ref, () => ({
      ...
      toggle: () => handleToggle(),
      validate: () => {
        const validation = validateSwitch(checked);
        setIsValid(validation.valid);
        setValidationMessage(validation.message);
    ...
    const renderSwitch = () => {
      ...
      return (
        <button
          ...
          type="button"
          className={cn(
            'relative inline-flex flex-shrink-0 cursor-pointer items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2',
            sizeStyles.switch,
        ...
        >
          ...
          <motion.span
            className={cn(
              'pointer-events-none rounded-full shadow-sm transition-all',
              sizeStyles.thumb,
              thumbBg
            )}
            initial={false}
            animate={{
              x: checked 
                ? `calc(${sizeStyles.switch.replace('h-', '').replace('w-', '').split(' ')[1] || '11'}px - ${sizeStyles.thumb.replace('h-', '').replace('w-', '').split(' ')[1] || '4.5'}px)`
                : 0,
