# ai/models/reinforcement/dqn_agent.py
"""
...
class _DQNNetwork(nn.Module):
    ...
    def _init_noisy_layers(self):
        """Initialise les couches NoisyNet"""
        pass
...
class DQNAgent:
    """
    DQN (Deep Q-Network) Agent for reinforcement learning in trading.
    ...
    def update_beta(self):
        """Met à jour beta pour la mémoire priorisée"""
        if self.config.prioritized_memory:
            self.config.beta = min(
                1.0,
                self.config.beta + self.config.beta_increment
            )
