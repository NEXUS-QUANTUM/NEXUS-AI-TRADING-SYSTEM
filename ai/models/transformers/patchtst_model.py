...
# ai/models/transformers/patchtst_model.py
"""
NEXUS AI TRADING SYSTEM - PatchTST Model for Time Series Forecasting
...
class PatchTSTResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'patches': self.patches.tolist() if isinstance(self.patches, np.ndarray) else self.patches,
...
class _PatchTSTModel(nn.Module):
    ...
    def __init__(self, config: PatchTSTConfig):
        ...
        self.n_patches = max(1, (config.context_length - config.patch_len) // config.stride + 1)

        # Projection des patches
        self.patch_proj = nn.Linear(config.patch_len * config.input_size, config.hidden_size)

        # Positional encoding
        if config.use_pe:
            self.pe = nn.Parameter(torch.randn(1, self.n_patches, config.hidden_size) * 0.1)

        # Encodeur
        self.encoder = _PatchTSTEncoder(config)
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(config.hidden_size // 2, config.prediction_length * config.output_size)
        )
    ...
    def forward(self, x):
        ...
        # Reshape
        x = x.view(batch_size, self.config.prediction_length, self.config.output_size)

        return x

    def _create_patches(self, x):
        """Crée des patches à partir de la séquence"""
        ...
        if self.padding_patch == 'end' and len(patches) < self.n_patches:
            last_patch = patches[-1]
            pad_len = self.patch_len - (seq_len - (len(patches) - 1) * self.stride)
            if pad_len > 0:
                last_patch = F.pad(last_patch, (0, 0, 0, pad_len, 0, 0))
...
class PatchTSTModel:
    ...
    PatchTST is a transformer-based model that processes time series as
    patches (subsequences) for efficient and effective forecasting.

    Features:
    - Patching of time series for better representation
    ...
    def __init__(self, config: Optional[PatchTSTConfig] = None):
        ...
        self.model: Optional[_PatchTSTModel] = None
        ...
        logger.info(f"PatchTSTModel initialisé sur {self.device}")
    ...
    def _denormalize_data(self, data: np.ndarray) -> np.ndarray:
        if self.config.revin and self.mean is not None and self.std is not None:
    ...
    def fit(
        ...
        **kwargs
        ...
        Returns:
            PatchTSTModel: Instance entraînée
        """
        ...
        normalized_val = None
        if validation_data is not None:
            normalized_val = (validation_data - self.mean) / self.std if self.config.revin else validation_data
        ...
        self.model = _PatchTSTModel(self.config).to(self.device)
    ...
    def predict(
    ...
    ) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray], PatchTSTResult]:
        """
        Effectue une prédiction avec le modèle PatchTST.

        Args:
            data: Données d'entrée
            prediction_length: Nombre d'étapes à prédire
            return_patches: Retourner les patches
            return_details: Retourner tous les détails

        Returns:
            np.ndarray: Prédictions
            Tuple: (Prédictions, Patches)
            PatchTSTResult: Résultat complet
        """
        ...
        cache_key = f"{hash(data.tobytes())}_{prediction_length}_{return_patches}"
        if cache_key in self._prediction_cache:
            ...
            if return_details:
                ...
                return cached.predictions, cached.patches
            else:
        ...
        if return_details:
            ...
            return predictions, result.patches
        else:
            return predictions
    ...
    def get_metrics(self) -> Dict[str, Any]:
        metrics = {
            ...
            'n_patches': self.model.n_patches if self.model else 0,
            'revin': self.config.revin,
    ...
    def load(cls, filepath: str) -> 'PatchTSTModel':
        try:
            ...
            config = PatchTSTConfig(**data['config'])
            model = cls(config)

            if data.get('model_state_dict'):
                model.model = _PatchTSTModel(config).to(model.device)
