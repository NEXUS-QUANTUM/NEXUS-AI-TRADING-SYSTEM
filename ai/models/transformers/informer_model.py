...
# ai/models/transformers/informer_model.py
"""
NEXUS AI TRADING SYSTEM - Informer Model for Time Series Forecasting
...
class _ProbSparseAttention(nn.Module):
    """Attention probabiliste parcimonieuse pour Informer"""

    def __init__(self, hidden_size: int, num_heads: int, factor: int = 5):
        super().__init__()
    ...
    def forward(self, queries, keys, values):
        ...
        q = self.q_proj(queries).view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        # Attention probabiliste parcimonieuse simplifiée
...
class _InformerEncoder(nn.Module):
    """Encodeur Informer avec distilling"""
    ...
    def __init__(self, config: InformerConfig):
        ...
        for _ in range(config.num_layers):
            attention = _ProbSparseAttention(
                config.hidden_size,
                config.num_heads,
                config.factor
            )
            ...
            self.norm_layers.append(nn.LayerNorm(config.hidden_size))

    def forward(self, x):
        for i in range(self.num_layers):
            # Attention
            attn_out, attn = self.attention_layers[i](x, x, x)
            x = self.norm_layers[i](x + attn_out)

            # Feed-forward
            ff_out = self.ff_layers[i](x)
            x = self.norm_layers[i](x + ff_out)
...
class _InformerDecoder(nn.Module):
    ...
    def __init__(self, config: InformerConfig):
        ...
        for _ in range(config.num_layers):
            ...
            cross_attention = _ProbSparseAttention(
                config.hidden_size,
                config.num_heads,
                config.factor
            )
    ...
    def forward(self, x, encoder_output):
        for i in range(self.num_layers):
            ...
            # Cross-attention avec encodeur
            cross_attn_out, _ = self.cross_attention_layers[i](x, encoder_output, encoder_output)
            x = self.norm_layers[i](x + cross_attn_out)
...
class _InformerModel(nn.Module):
    """Modèle Informer complet"""
    ...
    def __init__(self, config: InformerConfig):
        ...
        self.encoder = _InformerEncoder(config)
        self.decoder = _InformerDecoder(config)
...
class InformerModel:
    ...
    Informer is a transformer variant designed for long-term time series
    forecasting with probabilistic sparse attention and distilling.

    Features:
    - Probabilistic sparse attention for efficiency
    - Distilling for longer sequences
    ...
    def __init__(self, config: Optional[InformerConfig] = None):
        ...
        self.model: Optional[_InformerModel] = None
        ...
        logger.info(f"InformerModel initialisé sur {self.device}")
    ...
    def fit(
        ...
        **kwargs
        ...
        Returns:
            InformerModel: Instance entraînée
        """
        ...
        self.model = _InformerModel(self.config).to(self.device)
    ...
    def predict(
    ...
    ) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray], InformerResult]:
        """
        Effectue une prédiction avec le modèle Informer.
        ...
        Returns:
            ...
            InformerResult: Résultat complet
        """
    ...
    def get_metrics(self) -> Dict[str, Any]:
        metrics = {
            ...
            'prob_sparse': self.config.prob_sparse,
    ...
    @classmethod
    def load(cls, filepath: str) -> 'InformerModel':
        try:
            with open(filepath, 'rb') as f:
                data = pickle.load(f)

            config = InformerConfig(**data['config'])
            model = cls(config)

            if data.get('model_state_dict'):
                model.model = _InformerModel(config).to(model.device)
