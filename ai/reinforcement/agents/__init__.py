...
# ai/reinforcement/agents/__init__.py
"""
NEXUS AI TRADING SYSTEM - Reinforcement Learning Agents Module
Copyright © 2026 NEXUS QUANTUM LTD

Module d'agents d'apprentissage par renforcement.
"""

import logging
from typing import Optional, List, Dict, Any

from ai.reinforcement.agents.dqn_agent import (
    DQNAgent,
    DQNConfig,
    DQNResult,
    create_dqn_agent,
...
from ai.reinforcement.agents.ppo_agent import (
    PPOAgent,
    PPOConfig,
    PPOResult,
    create_ppo_agent,
...
from ai.reinforcement.agents.rainbow_agent import (
    RainbowAgent,
    RainbowConfig,
    RainbowResult,
...
from ai.reinforcement.agents.sac_agent import (
    SACAgent,
    SACConfig,
    SACResult,
    create_sac_agent,
...
from ai.reinforcement.agents.td3_agent import (
    TD3Agent,
    TD3Config,
    TD3Result,
    create_td3_agent,
...
def create_rl_agent(
    ...
    **kwargs
    ...
    Args:
        agent_type: Type d'agent ('dqn', 'ppo', 'rainbow', 'sac', 'td3')
    ...
    if agent_type == 'dqn':
        ...
        action_dim = kwargs.get('action_dim', 3)
        hidden_dim = kwargs.get('hidden_dim', 256)
        num_atoms = kwargs.get('num_atoms', 51)
        memory_size = kwargs.get('memory_size', 100000)
        
        return create_rainbow_agent(
            state_dim=state_dim,
            action_dim=action_dim,
            hidden_dim=hidden_dim,
            num_atoms=num_atoms,
...
def get_available_rl_agents() -> List[str]:
    ...
    """
    agents = ['dqn', 'ppo', 'rainbow', 'sac', 'td3']
...
def get_rl_agent_info(agent_type: str) -> Dict[str, Any]:
    ...
    info = {
        ...
        'rainbow': {
            'name': 'Rainbow DQN',
            'description': 'Combinaison des améliorations DQN',
            'advantages': [
                'Très performant',
                'Support des actions discrètes',
                'Multiple améliorations combinées'
            ],
            'disadvantages': [
            ...
            'use_cases': [
                'Environnements discrets complexes',
                'Quand la performance est critique',
                'Grands espaces d\'actions'
            ],
            'action_space': 'discrete',
        },
        'sac': {
...
def get_rl_agent_recommendation(
    ...
    recommendations = {
        'discrete': {
            ...
            'complex': {'agent': 'rainbow', 'description': 'Rainbow pour environnements complexes discrets'},
        },
        'continuous': {
    ...
    if stability == 'high' and recommendation['agent'] in ['dqn', 'rainbow']:
        if action_space_lower in ['discrete', 'both']:
...
logger.info("Module d'agents RL initialisé")
